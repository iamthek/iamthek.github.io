...
function create(){
    ...
    gameLoadState={
        preload:function(){
        game.stage.backgroundColor = '#3B5AB5';  
        gamelabel = game.add.text(w/2, h/2, '加载中...', { font: '30px Arial', fill: '#fff',align: 'center'  });
        gamelabel.anchor.setTo(0.5, 0.4);
        game.load.spritesheet('player', 'style/imgs/player_my.png', 32, 32);
        game.load.spritesheet('enemy1', 'style/imgs/enemy1.png', 32, 36);
        game.load.spritesheet('enemy2', 'style/imgs/enemy2.png', 28, 40);  
       // game.load.image('bg', 'style/imgs/background-wall.png');  
        },
        create:function(){
            gamelabel.text="这个页面太无趣\n无聊人士解救专用窗口\n请按键盘↑键";
    ...
    gameOverState={
        create: function () {
        gamelabel = game.add.text(w/2, h/2, 'GG思密达\n\n分数: '+score+'\n历史最高分:69\n解救小祥子按键盘↑键\n重新开始', { font: '30px Arial', fill: '#fff', align: 'center' });
    ...
    gamePalayState={
    create: function () {
      //  game.add.tileSprite(0, 0, 800, 400, 'bg');
		this.cursor = game.input.keyboard.createCursorKeys();
